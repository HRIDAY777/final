...
class StudentPerformance(models.Model):
    ...
    def overall_performance_score(self):
        ...
        exam_score = (
            sum(self.exam_scores) / len(self.exam_scores)
            if self.exam_scores else 0
        )
    ...
    def save(self, *args, **kwargs):
        if self.total_students > 0:
            self.attendance_rate = (
                self.present_count / self.total_students
            ) * 100
            self.absence_rate = (
                self.absent_count / self.total_students
            ) * 100
...
class ExamAnalytics(models.Model):
    ...
    exam = models.ForeignKey(
        'exams.Exam', on_delete=models.CASCADE, related_name='analytics'
    )
    ...
    question_analysis = models.JSONField(
        default=list
    )  # List of question performance data
...
class LearningAnalytics(models.Model):
    ...
    learning_path_progress = models.DecimalField(
        max_digits=5, decimal_places=2, default=0
    )
...
class PredictiveAnalytics(models.Model):
    ...
    # Predictions
    predicted_gpa = models.DecimalField(
        max_digits=3, decimal_places=2,
        validators=[MinValueValidator(0), MaxValueValidator(4)]
    )
    graduation_probability = models.DecimalField(
        max_digits=5, decimal_places=2,
        validators=[MinValueValidator(0), MaxValueValidator(100)]
    )
    dropout_risk = models.DecimalField(
        max_digits=5, decimal_places=2,
        validators=[MinValueValidator(0), MaxValueValidator(100)]
    )
...
class AnalyticsDashboard(models.Model):
    ...
    allowed_users = models.ManyToManyField(
        User, blank=True, related_name='accessible_dashboards'
    )
    ...
    created_by = models.ForeignKey(
        ...
        related_name='created_analytics_dashboards'
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
