...
def send_invoice_notification(sender, instance, created, **kwargs):
    ...
    if created and settings.EMAIL_HOST:
        try:
            ...
            if (hasattr(instance.tenant, 'admin_email') and
                    instance.tenant.admin_email):
                send_mail(
                    subject=subject,
                    message=message,
...
def send_payment_confirmation(sender, instance, created, **kwargs):
    ...
    if created and instance.status == 'completed' and settings.EMAIL_HOST:
        try:
            ...
            message = f"""
            Dear {instance.paid_by.get_full_name()
                  if instance.paid_by else 'Customer'},
...
def send_subscription_notification(sender, instance, created, **kwargs):
    ...
    if created and settings.EMAIL_HOST:
        try:
            subject = (f"Subscription {instance.get_status_display()} - "
                       f"{instance.plan.name}")
            message = f"""
            Dear {instance.tenant.name},
    ...
    for invoice in overdue_invoices:
        if settings.EMAIL_HOST:
            try:
                ...
                if (hasattr(invoice.tenant, 'admin_email') and
                        invoice.tenant.admin_email):
                    send_mail(
                        subject=subject,
                        message=message,
                        from_email=settings.DEFAULT_FROM_EMAIL,
