...
class AIModel(models.Model):
    ...
    hyperparameters = models.JSONField(
        default=dict, help_text="Training hyperparameters"
    )
    model_path = models.CharField(
        max_length=500, blank=True, help_text="Path to the model file"
    )

    # Performance metrics
    accuracy = models.FloatField(
        null=True, blank=True, help_text="Model accuracy score"
    )
    precision = models.FloatField(
        null=True, blank=True, help_text="Model precision score"
    )
    recall = models.FloatField(
        null=True, blank=True, help_text="Model recall score"
    )
    f1_score = models.FloatField(
        null=True, blank=True, help_text="Model F1 score"
    )
    ...
    created_by = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name='created_ai_models'
    )
    shared_with = models.ManyToManyField(
        User, blank=True, related_name='shared_ai_models'
    )
...
class AIQuizGenerator(models.Model):
    ...
    subject = models.ForeignKey(
        'subjects.Subject', on_delete=models.CASCADE, related_name='ai_quizzes'
    )
    syllabus_topic = models.CharField(max_length=200, blank=True)
    content_source = models.TextField(
        help_text="Source content for quiz generation"
    )
    ...
    generation_params = models.JSONField(
        default=dict, help_text="Parameters used for quiz generation"
    )

    # Status and tracking
    is_generated = models.BooleanField(default=False)
    generation_status = models.CharField(
        max_length=20, choices=AIModel.STATUS_CHOICES, default='inactive'
    )
    ...
    shared_with = models.ManyToManyField(
        User, blank=True, related_name='shared_quizzes'
    )
...
class AIQuestion(models.Model):
    ...
    quiz = models.ForeignKey(
        AIQuizGenerator, on_delete=models.CASCADE, related_name='questions'
    )

    # Question content
    question_text = models.TextField()
    question_type = models.CharField(
        max_length=20, choices=AIQuizGenerator.QUESTION_TYPES
    )
    difficulty = models.CharField(
        max_length=20, choices=AIQuizGenerator.DIFFICULTY_CHOICES
    )

    # Options and answers
    options = models.JSONField(
        default=list, help_text="Multiple choice options"
    )
    correct_answer = models.TextField(
        help_text="Correct answer or answer key"
    )
    ...
    order = models.IntegerField(
        default=0, help_text="Question order in quiz"
    )
    ...
    confidence_score = models.FloatField(
        ...
        help_text="AI confidence in this question"
    )
...
class AILessonSummarizer(models.Model):
    ...
    lesson = models.ForeignKey(
        'academics.Lesson', on_delete=models.CASCADE,
    ...
    summary_type = models.CharField(
        max_length=20, choices=SUMMARY_TYPES, default='detailed'
    )
    summary_content = models.TextField()
    key_points = models.JSONField(
        default=list, help_text="List of key points from the lesson"
    )
    vocabulary = models.JSONField(
        default=list, help_text="Important vocabulary terms"
    )
    ...
    generation_params = models.JSONField(
        default=dict, help_text="Parameters used for summary generation"
    )

    # Quality metrics
    readability_score = models.FloatField(
        null=True, blank=True,
        help_text="Readability score of the summary"
    )
    coherence_score = models.FloatField(
        null=True, blank=True,
        help_text="Coherence score of the summary"
    )
    ...
    # Access control
    created_by = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name='created_summaries'
    )
    shared_with = models.ManyToManyField(
        User, blank=True, related_name='shared_summaries'
    )
...
class AIPerformancePredictor(models.Model):
    ...
    subject = models.ForeignKey(
        'subjects.Subject', on_delete=models.CASCADE,
        related_name='ai_predictions', null=True, blank=True
    )

    # Prediction details
    prediction_type = models.CharField(max_length=20, choices=PREDICTION_TYPES)
    predicted_value = models.FloatField(
        help_text="Predicted score or probability"
    )
    ...
    generation_params = models.JSONField(
        default=dict, help_text="Parameters used for prediction"
    )
    ...
    actual_value = models.FloatField(
        null=True, blank=True, help_text="Actual value when available"
    )
    prediction_accuracy = models.FloatField(
        null=True, blank=True, help_text="Accuracy of prediction"
    )
    ...
    def __str__(self):
        return (f"{self.student.full_name} - "
                f"{self.get_prediction_type_display()}")
...
class AIAttendanceAnomalyDetector(models.Model):
    ...
    confidence_score = models.FloatField(
        help_text="Confidence in anomaly detection (0-1)"
    )

    # Detection data
    detection_date = models.DateField()
    attendance_rate = models.FloatField(help_text="Current attendance rate")
    historical_average = models.FloatField(
        help_text="Historical average attendance rate"
    )
    deviation = models.FloatField(
        help_text="Deviation from historical average"
    )

    # Analysis
    contributing_factors = models.JSONField(
        default=list, help_text="Factors contributing to the anomaly"
    )
    pattern_analysis = models.JSONField(
        default=dict, help_text="Pattern analysis data"
    )
    peer_comparison = models.JSONField(
        default=dict, help_text="Comparison with peer group"
    )
    ...
    detection_params = models.JSONField(
        default=dict, help_text="Parameters used for anomaly detection"
    )

    # Actions and recommendations
    recommendations = models.JSONField(
        default=list, help_text="Recommended actions"
    )
    intervention_needed = models.BooleanField(default=False)
    intervention_actions = models.JSONField(
        default=list, help_text="Specific intervention actions"
    )
...
class AINaturalLanguageQuery(models.Model):
    ...
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name='nlq_queries'
    )

    # Query details
    query_text = models.TextField(
        help_text="Natural language query from user"
    )
    query_type = models.CharField(max_length=20, choices=QUERY_TYPES)
    intent = models.CharField(
        max_length=100, blank=True, help_text="Detected intent of the query"
    )

    # Processing
    processed_query = models.JSONField(
        default=dict, help_text="Processed query structure"
    )
    sql_query = models.TextField(blank=True, help_text="Generated SQL query")
    parameters = models.JSONField(
        default=dict, help_text="Query parameters"
    )

    # Results
    result_data = models.JSONField(default=dict, help_text="Query results")
    result_summary = models.TextField(
        blank=True, help_text="Natural language summary of results"
    )
    visualization_config = models.JSONField(
        default=dict, help_text="Configuration for result visualization"
    )

    # Model and processing
    ai_model = models.ForeignKey(
        AIModel, on_delete=models.SET_NULL, null=True, blank=True,
        related_name='nlq_queries'
    )
    processing_params = models.JSONField(
        default=dict, help_text="Parameters used for query processing"
    )
    ...
    user_feedback = models.TextField(
        blank=True, help_text="User feedback on results"
    )
...
class AITrainingJob(models.Model):
    ...
    ai_model = models.ForeignKey(
        AIModel, on_delete=models.CASCADE, related_name='training_jobs'
    )
    training_config = models.JSONField(
        default=dict, help_text="Training configuration"
    )
    dataset_config = models.JSONField(
        default=dict, help_text="Dataset configuration"
    )
    ...
    progress = models.IntegerField(
        default=0, help_text="Training progress percentage"
    )
    ...
    training_metrics = models.JSONField(
        default=dict, help_text="Training metrics over time"
    )
    validation_metrics = models.JSONField(
        default=dict, help_text="Validation metrics over time"
    )
    final_metrics = models.JSONField(
        default=dict, help_text="Final model metrics"
    )
    ...
    gpu_usage = models.FloatField(
        null=True, blank=True, help_text="GPU usage percentage"
    )
    memory_usage = models.FloatField(
        null=True, blank=True, help_text="Memory usage in GB"
    )
    ...
    # Access control
    created_by = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name='created_training_jobs'
    )
    ...
    def __str__(self):
        return (f"{self.name} - {self.ai_model.name} "
...
class AIDataSource(models.Model):
    ...
    schema_config = models.JSONField(
        default=dict, help_text="Data schema configuration"
    )

    # Data quality
    data_quality_score = models.FloatField(
        null=True, blank=True, help_text="Data quality score (0-1)"
    )
    ...
    created_by = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name='created_data_sources'
    )
    shared_with = models.ManyToManyField(
        User, blank=True, related_name='shared_data_sources'
    )
...
class AIUsageLog(models.Model):
    ...
    input_data = models.JSONField(
        default=dict, help_text="Input data provided to AI tool"
    )
    output_data = models.JSONField(
        default=dict, help_text="Output data from AI tool"
    )
    processing_time = models.DurationField(null=True, blank=True)
    ...
    error_message = models.TextField(
        blank=True, help_text="Error message if failed"
    )
    user_satisfaction = models.IntegerField(
        null=True, blank=True, help_text="User satisfaction rating (1-5)"
    )
    ...
    def __str__(self):
        return (f"{self.user.username} - {self.get_tool_type_display()} - "
                f"{self.timestamp}")
